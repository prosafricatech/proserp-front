...
- **Gender** (Male/Female), **Department** (live list), **Employment Type**
  (Full Time/Part Time/Casual), **Designation** (live list from the Designations reference sheet).
- Columns: Employee Number, First Name*, Middle Name, Last Name*, Gender, Email,
  ...
  Employment Type, Join Date, **Basic Salary**, **Contract Start Date**, **Designation**.
  (\* required.)
...
**UI:**
1. "Download template" button → downloads Data sheet + Departments + Designations reference
2. User fills in employee data (can use names from reference sheets or enter new ones)
3. Upload form:
   - File picker
...
**Scopes (deductions/contributions):** `all` applies to all imported employees; `active_contracts` applies only to those
...
**Auto-creation (data cleanup):**
- **Department** — if a department name is filled but doesn't exist in the system, it's automatically created
- **Designation** — if a designation is filled but doesn't exist in the system, it's automatically created with that title
- This prevents "unknown department/designation" errors and keeps the import fast — no manual setup needed upfront

**Bonus:** if a row has **Basic Salary**, the import creates a starter active **contract** for that
employee (permanent, status = active, basic_salary from the column). If a **Designation** is also
filled, it's assigned to the contract and auto-created if needed. They're immediately ready for payroll.
(Without a contract, an employee can't appear in a payroll run.)
...
# Part 4 — Employee Audit Trails (Movements & Salary History)

The system automatically tracks all changes to employee positions and salaries for complete audit trails.

## 4.1 Cost Center & Department Movements

**What is tracked:**
- When an employee moves from one cost center to another (e.g., HQ → Branch A)
- When an employee changes departments (e.g., Sales → Operations)
- Auto-recorded with date, who made the change, and optional reason

**When it happens:**
- Automatically logged when you update `employee.cost_center_id` or `employee.department_id`
- Include a `reason` field for context (e.g., "Site relocation", "Team restructuring")

**API calls to view history:**

| Action | Call |
|---|---|
| Cost center movements | `GET /employees/{employeeId}/cost-center-movements` |
| Department movements | `GET /employees/{employeeId}/department-movements` |
| Both (combined) | `GET /employees/{employeeId}/all-movements` |
...
```json
{
  "data": [
    {
      "id": 1,
      "from_cost_center_id": 5,
      "to_cost_center_id": 8,
      "moved_date": "2026-06-15",
      "reason": "Site relocation - opened new branch",
      "created_by": 12,
...
**What is tracked:**
- Every change to `employee_contract.basic_salary` (raises, downgrades, adjustments)
- Auto-recorded with from/to amounts, effective date, who made the change, and optional reason

**When it happens:**
- Automatically logged when you update the contract's basic salary
- Include a `reason` field for context (e.g., "Annual raise", "Promotion", "Demotion")

**API calls to view history:**

| Action | Call |
|---|---|
| One contract's salary history | `GET /employee-contracts/{contractId}/salary-history` |
| All contracts for an employee | `GET /employees/{employeeId}/salary-history` |
...
```json
{
  "contract": {
    ...
    "current_basic_salary": 600000
  },
  "salary_changes": [
    {
      "id": 5,
      "from_basic_salary": 500000,
      "to_basic_salary": 600000,
...
**For employee edit screen:**
1. Add optional **Reason** text field when updating `cost_center_id` or `department_id`
2. Add optional **Reason** text field when updating `basic_salary` on contracts
3. On employee detail page, add tabs/sections to show:
   - **Movements** (cost center + department history)
   - **Salary** (all salary changes across all contracts)

**Data display:**
- Show chronologically (newest first)
- Display: From → To, Date, Reason, Changed By (user name)
- Salary changes show amount change clearly: "$500k → $600k on 2026-06-01"
