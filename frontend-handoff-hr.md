...
```json
{
  "salary_expense_ledger_id": 41,     // account for salary cost
...
1. **Salary component accounts (GL mapping).** For posting (Step 6) to work, each
...
2. **Approval chains.** The multi-step approval uses the **same approval-chain
...
`POST /employees/import_registration_excel` — `multipart/form-data`, field
**`employees_excel`** (`.xlsx`/`.xls`). Response:
```json
{
...
```
**UI:** a "Download template" button and an upload control. After upload, show the
imported/skipped counts and list the `errors` (row number + message) so the user can
fix those rows and re-upload. Re-uploading is safe — duplicates are skipped.

**Bonus:** if a row has **Basic Salary**, the import also creates a starter active
**contract** for that employee, so they're immediately ready for payroll. (Without a
contract, an employee can't appear in a payroll run.)
