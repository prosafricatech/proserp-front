...
export async function exportProjectLiabilitiesExcel(exportedData: any) {
  try {
    ...
    const ws = wb.addWorksheet('Project Liabilities');
    ...
    if (rows.length) {
      ...
      // Empty state — mirrors PDF's "No liabilities available" fallback
      const rowNum = (ws.lastRow?.number ?? 0) + 1;
      ws.addRow(['No liabilities available', ' ']);
    ...
    // ── TOTAL ROW ───────────────────────────────────────────────────────────
    // PDF: Total label (flex 2) | total value (flex 1, right-aligned), lightColor fill
    ...
    console.error('Error exporting Project Liabilities Excel:', error);
    throw new Error(
