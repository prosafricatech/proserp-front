...
import { Divider, Grid, IconButton, TextField, Tooltip, Typography } from '@mui/material';
import React from 'react';
import { useFormContext } from 'react-hook-form';
import DeleteIcon from '@mui/icons-material/Delete';

import { Button } from '@mui/material';
function ItemsTab({purchase_order_items, onRemoveItem, onResetItems, canReset}) {
      ...
      {purchase_order_items?.map((item, index) => (
        <React.Fragment key={item.id}>
          <Grid container sx={{ cursor: 'pointer', '&:hover': { bgcolor: 'action.hover' } }}>
            ...
            <Grid size={0.5}><Div sx={{ mt: 1.7, mb: 1.7 }}>{index + 1}.</Div></Grid>
