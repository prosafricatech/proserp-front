...
const SalesAndCashSummary: React.FC<SalesAndCashSummaryProps> = ({
  setOpenSalesAndCashSummary,
}) => {
  ...
  const retrieveReport = async (filters: {
    ...
    try {
      const report =
        await financialReportsServices.fetchSalesAndCashSummary(filters);
            ...
            >
              <Grid size={{ xs: 12, md: 4, lg: 5 }}>
                <CostCenterSelector
                  ...
                  allowSameType={true}
                  onChange={(
                    cost_centers: CostCenter | CostCenter[] | null
                  ) => {
                    const selectedCostCenters =
                      cost_centers || authOrganization?.costCenters || [];
                    const selectedCostCenterIds = selectedCostCenters.map(
                      (cost_center: CostCenter) => cost_center.id
                    );
