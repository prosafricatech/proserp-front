...
interface SaleInvoiceTopInformationProps {
...
const SaleInvoiceTopInformation: React.FC<SaleInvoiceTopInformationProps> = ({
  sale,
  setValue,
  register,
  watch,
  ...
  // Fetch delivery notes - only if sale exists
  const { data: saleDeliveryNotes, isLoading: isDeliveryNotesLoading } = useQuery<DeliveryNote[]>({
    ...
    enabled: !!sale?.id && !sale?.is_instant_sale
  });
      ...
      <Grid size={{ xs: 12, md: 9 }}>
        <form autoComplete='off'>
          <Grid container spacing={2}>
            ...
            {organization?.is_tra_connected && (!organization?.is_vat_registered || (sale?.vat_amount || 0) > 0) && (
              <Grid size={12}>
