...
const SaleAdjustments: React.FC<SaleAdjustmentsProps> = ({
  expanded,
  sale,
  activeTab,
}) => {
  ...
  const [openAdjustmentDeleteDialog, setOpenAdjustmentDeleteDialog] =
    useState(false);
  const [openAdjustmentEditDialog, setOpenAdjustmentEditDialog] =
    useState(false);
            ...
            >
              <Grid size={{ xs: 6, md: 3, lg: 3 }}>
                <Tooltip title='Transaction Date'>
                  <Typography>
                    {readableDate(adjustment.transaction_date)}
                  </Typography>
                </Tooltip>
              </Grid>
              ...
              <Grid size={{ xs: 12, md: 2, lg: 2 }}>
                ...
                >
                  <Tooltip
                    title={
                      belowLargeScreen
                        ? `Download Adjustment ${adjustment.VoucherNo}`
                        : `View Adjustment ${adjustment.VoucherNo}`
                    }
                  >
                    <IconButton
                      onClick={() => {
