...
const SalesDispatchReport = lazy(
  () => import('./saleDispatchReport/SalesDispatchReport')
);
      ...
      <DialogTitle>
        {belowLargeScreen && (
          <Tabs
            ...
            aria-label='dispatch report tabs'
          >
            <Tab label='ONSCREEN' />
            <Tab label='PDF' />
          </Tabs>
...
const SalesListItemTabs: React.FC<SalesListItemTabsProps> = ({
  ...
  const createReceipts =
    accountsPersonnel ||
    checkOrganizationPermission([PERMISSIONS.RECEIPTS_CREATE]);
    ...
    <>
          ...
          <Grid size={12} sx={{ textAlign: 'right' }}>
            ...
            {!sale.is_instant_sale &&
              ...
              (checkOrganizationPermission([
                PERMISSIONS.SALES_DISPATCH_NOT_FULLY_PAID,
              ]) ||
                (checkOrganizationPermission([PERMISSIONS.SALES_DISPATCH]) &&
                  !!sale.is_fully_paid)) &&
              (checkOrganizationPermission([
                PERMISSIONS.SALES_DISPATCH_OLD_SALES,
              ]) ||
      ...
      {accountsPersonnel &&
        ...
        activeTab ===
          (!sale.is_instant_sale
            ? 3
            : sale.payment_method === 'On Account'
              ? 2
              : 1) &&
        !!sale.is_invoiced && (
      ...
      >
        {checkOrganizationPermission(PERMISSIONS.SALES_DISPATCH) ? (
          <DispatchDialog
            ...
            toggleOpen={setOpenDispatchDialog}
          />
        ) : (
          <UnauthorizedAccess />
        )}
      ...
      >
        {checkOrganizationPermission(PERMISSIONS.SALES_EDIT) ? (
          <InvoicesDialog
            saleId={Number(sale.id)}
            toggleOpen={setOpenInvoicesDialog}
          />
        ) : (
          <UnauthorizedAccess />
        )}
      ...
      >
        {checkOrganizationPermission(PERMISSIONS.SALES_EDIT) ? (
          <ReceiptDialog
            saleId={Number(sale.id)}
            toggleOpen={setOpenReceiptDialog}
          />
        ) : (
          <UnauthorizedAccess />
        )}
