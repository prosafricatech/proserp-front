...
const IncomeStatementOnScreen = ({ reportData }) => {
    ...
    const directExpenses = reportData?.directExpenses || reportData?.direct_expenses || [];
    const indirectExpenses = reportData?.indirectExpenses || reportData?.indirect_expenses || [];
    ...
    const getLedgerTotal = (ledger) => {
        ...
        return ledger.amounts.reduce((acc, item) => acc + (Number(item?.amount) || 0), 0);
    };
    ...
    const getSectionPeriodTotal = (items, period) => {
        ...
        return items.reduce((acc, ledger) => acc + getAmountByPeriod(ledger, period), 0);
    };

    // Totals based on new amounts[] response structure
    const totalRevenue = incomes.reduce((acc, curr) => acc + getLedgerTotal(curr), 0);
    const totalCostOfRevenue = directExpenses.reduce((acc, curr) => acc + getLedgerTotal(curr), 0);
    const totalOperatingExpenses = indirectExpenses.reduce((acc, curr) => acc + getLedgerTotal(curr), 0);

    const [ledgerDialogOpen, setLedgerDialogOpen] = useState(false);
    const [ledgerFilters, setLedgerFilters] = useState(null);

    const handleViewLedger = (ledgerId, ledger_name, increasesWith, fromDate, toDate) => {
        setLedgerFilters({
        ...
        <>
            <TableContainer component={Paper} sx={{ overflowX: 'auto', maxHeight: '70vh' }}>
                ...
                >
                    <TableHead>
                        <TableRow>
                            <TableCell sx={{ ...categoryCellSx, fontWeight: 'bold' }}>CATEGORY</TableCell>
                            {periods.map((periodItem) => (
                                <TableCell key={`period-header-${periodItem.period}`} align="right" sx={{ fontWeight: 'bold' }}>
                                    <Box sx={{ fontWeight: 'bold' }}>{periodItem.period}</Box>
                                    <Box sx={{ fontSize: '0.7rem', color: 'text.secondary', fontWeight: 'bold' }}>
                    ...
                    <TableBody>
                        ...
                        >
                            ...
                            {periods.map((periodItem) => {
                                const tooltipTitle = periodItem.start_datetime && periodItem.end_datetime
                                    ? `${formatDateTime(periodItem.start_datetime)} - ${formatDateTime(periodItem.end_datetime)}`
                                    : '';
                                return (
                                    <TableCell key={`revenue-total-${periodItem.period}`} align="right" sx={{ fontWeight: 'bold' }}>
                                        <Tooltip title={tooltipTitle} placement="top" arrow>
                                            <span>
                                                {getSectionPeriodTotal(incomes, periodItem.period).toLocaleString('en-US', {
                        ...
                        {openRows.revenue && incomes.map((component, index) => (
                            <TableRow key={`revenue-ledger-${index}`} sx={{ '&:hover': { bgcolor: 'action.hover' } }}>
                                ...
                                {periods.map((periodItem) => {
                                    const amountItem = getAmountItemByPeriod(component, periodItem.period);
                                    const timeframe = amountItem && amountItem.start_datetime && amountItem.end_datetime
                                        ? `\n${formatDateTime(amountItem.start_datetime)} - ${formatDateTime(amountItem.end_datetime)}`
                                        : '';
                                    const tooltipTitle = `${component.ledger_name}${timeframe}`;
                                    return (
                                        <TableCell key={`revenue-ledger-${index}-${periodItem.period}`} align="right">
                                            <Tooltip title={<span style={{ whiteSpace: 'pre-line' }}>{tooltipTitle}</span>} placement="top" arrow>
                                                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 0.5 }}>
                                                    ...
                                                    {amountItem && (
                                                        <Tooltip title={`View ${component.ledger_name} - ${periodItem.period}`}>
                                                            <IconButton
                                                                size="small"
                                                                onClick={() => {
                                                                    handleViewLedger(
                        ...
                        >
                            ...
                            {periods.map((periodItem) => {
                                const tooltipTitle = periodItem.start_datetime && periodItem.end_datetime
                                    ...
                                    : '';
                                return (
                                    <TableCell key={`cost-total-${periodItem.period}`} align="right" sx={{ fontWeight: 'bold' }}>
                                        <Tooltip title={tooltipTitle} placement="top" arrow>
                                            <span>
                                                {getSectionPeriodTotal(directExpenses, periodItem.period).toLocaleString('en-US', {
                                                    minimumFractionDigits: 2,
                                                    maximumFractionDigits: 2,
                                                })}
                                            </span>
                        ...
                        {openRows.costOfRevenue && directExpenses.map((component, index) => (
                            <TableRow key={`cost-ledger-${index}`} sx={{ '&:hover': { bgcolor: 'action.hover' } }}>
                                <TableCell sx={{ ...categoryCellSx, pl: 4, paddingLeft: 6 }}>
                                    <Box display="flex" alignItems="center">
                                        {component.ledger_name}
                                    </Box>
                        ...
                        <TableRow sx={{ cursor: 'pointer', '&:hover': { bgcolor: 'action.hover' } }}>
                            ...
                            {periods.map((periodItem) => {
                                ...
                                return (
                                    <TableCell key={`gross-period-${periodItem.period}`} align="right" sx={{ fontWeight: 'bold' }}>
                                        <Tooltip title={<span style={{ whiteSpace: 'pre-line' }}>{tooltipTitle}</span>} placement="top" arrow>
                                            <span>
                                                {(getSectionPeriodTotal(incomes, periodItem.period) - getSectionPeriodTotal(directExpenses, periodItem.period)).toLocaleString('en-US', {
                                                    minimumFractionDigits: 2,
                                                    maximumFractionDigits: 2,
                                                })}
                                            </span>
                        ...
                        >
                            ...
                            {periods.map((periodItem) => {
                                ...
                                return (
                                    <TableCell key={`operating-total-${periodItem.period}`} align="right" sx={{ fontWeight: 'bold' }}>
                                        <Tooltip title={<span style={{ whiteSpace: 'pre-line' }}>{tooltipTitle}</span>} placement="top" arrow>
                                            <span>
                                                {getSectionPeriodTotal(indirectExpenses, periodItem.period).toLocaleString('en-US', {
                                                    minimumFractionDigits: 2,
                                                    maximumFractionDigits: 2,
                                                })}
                                            </span>
                        ...
                        {openRows.operatingExpenses && indirectExpenses.map((component, index) => (
                            <TableRow key={`expense-ledger-${index}`} sx={{ '&:hover': { bgcolor: 'action.hover' } }}>
                                <TableCell sx={{ ...categoryCellSx, pl: 4, paddingLeft: 6 }}>
                                    <Box display="flex" alignItems="center">
                                        {component.ledger_name}
                                    </Box>
                        ...
                        <TableRow sx={{ cursor: 'pointer', '&:hover': { bgcolor: 'action.hover' } }}>
                            ...
                            {periods.map((periodItem) => {
                                ...
                                return (
                                    <TableCell key={`net-period-${periodItem.period}`} align="right" sx={{ fontWeight: 'bold' }}>
                                        <Tooltip title={<span style={{ whiteSpace: 'pre-line' }}>{tooltipTitle}</span>} placement="top" arrow>
                                            <span>
                                                {(
