              ...
              <TableHead>
                <TableRow>
                  ...
                  {allowanceTypes.map((type) => (
                    <TableCell key={`allowance-${type.id || type.name}`} align='right'>
                      {type.name || 'Allowance'}
                    </TableCell>
                  ))}
                  <TableCell align='right'>Gross</TableCell>
                  {preTaxDeductionTypes.map((type) => (
                    <TableCell key={`pre-tax-deduction-${type.id || type.name}`} align='right'>
                  ...
                  {postTaxDeductionTypes.map((type) => (
                    <TableCell key={`post-tax-deduction-${type.id || type.name}`} align='right'>
                  ...
                  {contributionTypes.map((type) => (
                    <TableCell key={`contribution-${type.id || type.name}`} align='right'>
              ...
              <TableBody>
                {rows.map((entry, index) => {
                  ...
                  return (
                    <TableRow key={entry.run.id}>
                      ...
                      {allowanceTypes.map((type) => (
                        <TableCell key={`allowance-value-${entry.run.id}-${type.id || type.name}`} align='right'>
                      ...
                      {preTaxDeductionTypes.map((type) => (
                        <TableCell key={`pre-tax-deduction-value-${entry.run.id}-${type.id || type.name}`} align='right'>
                          {fmt(sumDeductionByType(entry.run, type))}
                        </TableCell>
                      ))}

                      <TableCell align='right'>{fmt(entry.computed.taxableIncome)}</TableCell>
                      <TableCell align='right'>{fmt(entry.computed.paye)}</TableCell>

                      {postTaxDeductionTypes.map((type) => (
                        <TableCell key={`post-tax-deduction-value-${entry.run.id}-${type.id || type.name}`} align='right'>
                          {fmt(sumDeductionByType(entry.run, type))}
                        </TableCell>
                      ))}
                      ...
                      {contributionTypes.map((type) => (
                        <TableCell key={`contribution-value-${entry.run.id}-${type.id || type.name}`} align='right'>
                          {fmt(sumContributionByType(entry.run, type))}
                        </TableCell>
                      ))}

                      <TableCell align='right'>{fmt(entry.computed.totalEmployerContributions)}</TableCell>
                ...
                <TableRow>
                  ...
                  {totals.allowanceByType.map((amount, index) => (
                    <TableCell key={`allowance-total-${index}`} align='right' sx={{ fontWeight: 700, borderTop: '2px solid', borderColor: 'divider' }}>
                      {fmt(amount)}
                    </TableCell>
                  ))}
                  ...
                  {totals.preTaxDeductionByType.map((amount, index) => (
                    <TableCell key={`pre-tax-deduction-total-${index}`} align='right' sx={{ fontWeight: 700, borderTop: '2px solid', borderColor: 'divider' }}>
                      {fmt(amount)}
                    </TableCell>
                  ))}
                  ...
                  {totals.postTaxDeductionByType.map((amount, index) => (
                    <TableCell key={`post-tax-deduction-total-${index}`} align='right' sx={{ fontWeight: 700, borderTop: '2px solid', borderColor: 'divider' }}>
                      {fmt(amount)}
                    </TableCell>
                  ))}
                  ...
                  {totals.contributionByType.map((amount, index) => (
                    <TableCell key={`contribution-total-${index}`} align='right' sx={{ fontWeight: 700, borderTop: '2px solid', borderColor: 'divider' }}>
                      {fmt(amount)}
                    </TableCell>
                  ))}
