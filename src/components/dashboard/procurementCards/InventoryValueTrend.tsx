...
function InventoryValueTrend(props: InventoryValueTrendProps) {
  ...
  const dashboardFilters = useDashboardSettings?.()
    ? useDashboardSettings()
    : undefined;
  const from = props.from ?? dashboardFilters?.chartFilters?.from;
  const to = props.to ?? dashboardFilters?.chartFilters?.to;
  const cost_center_ids =
    props.cost_center_ids ?? dashboardFilters?.chartFilters?.cost_center_ids;
  ...
  const processInventoryValues = (
    data: InventoryValueItem[],
    aggregateBy: string
  ): ProcessedInventoryValue[] => {
