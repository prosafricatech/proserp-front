...
function DatabaseActions() {
  ...
  const [selectedData, setSelectedData] = useState<DatabaseFormData | null>(
    null
  );
  ...
  const validationSchema = yup.object({
    ...
    action: yup.string().required('Database action is required'),
    password: yup
      .string()
      .test(
        'required-if-dialog-open',
        'Please enter your password to confirm action',
        function (value) {
          if (openDialog && !value) {
            return false;
          }
  ...
  const runMigration = useMutation({
    mutationFn: (data: DatabaseActionRequest) =>
      supportServices.runDatabaseActions(data),
    onSuccess: (data) => {
      if (data?.message) {
    ...
    <>
          ...
          <Grid size={12}>
            <Autocomplete
              ...
              onChange={(event, newValue: OrganizationOption[]) => {
                ...
                const allOrganizations = newValue.find(
                  (db) => db.id === 'organizations'
                );
                ...
                setValue('databases', databases.length > 0 ? databases : [], {
                  shouldValidate: true,
                  shouldDirty: true,
                });
              }}
                ...
                {checkPermission([
                  ...
                  <>
                    ...
                    <FormControlLabel
                      value='removeTrashed'
                      control={<Radio />}
                      label='Database:RemoveTrashed'
                    />
                  </>
                )}
                {checkPermission(PROS_CONTROL_PERMISSIONS.DATABASE_REFRESH) &&
                  watch('databases').length > 0 && (
                    <>
                      {!watch('databases').find(
                        (database) => database.id === 'core'
                      ) && (
                        <FormControlLabel
                          value='refresh'
                          control={<Radio />}
                      ...
                      {watch('databases').find(
                        ...
                        <FormControlLabel
                          value='constantsReseed'
                          control={<Radio />}
                          label='Constants:Reseed'
                        />
                      )}
                    </>
                  )}
                {checkPermission(
                  PROS_CONTROL_PERMISSIONS.DATABASE_RUN_SCRIPTS
                ) && (
                  <FormControlLabel
                    value='runScripts'
                    control={<Radio />}
      ...
      >
        <DialogTitle sx={{ textAlign: 'center' }}>
          ...
          >
            {selectedData?.action === 'up'
              ? 'Migrate:Up?'
              : selectedData?.action === 'down'
                ? 'Migrate:Down?'
                : selectedData?.action === 'refresh'
                  ? 'Migrate:Refresh?'
                  : selectedData?.action === 'constantsReseed'
                    ? 'Constants:Reseed?'
                    : selectedData?.action === 'runScripts'
                      ? 'Run Scripts?'
                      : selectedData?.action === 'removeTrashed'
                        ? 'Database:RemoveTrashed?'
                        : 'Unknown Action'}
          </Typography>
