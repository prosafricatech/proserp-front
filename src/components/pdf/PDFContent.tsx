...
const StyledDownloadLink = styled('div')(({ theme }) => ({
  ...
  '& a': {
    color:
      theme.palette.mode === 'dark' ? '#29f096' : theme.palette.primary.main,
    textDecoration: 'none',
    fontWeight: 500,
    ...
    '&:hover': {
      color:
        theme.palette.mode === 'dark' ? '#1ed184' : theme.palette.primary.dark,
      backgroundColor:
      ...
      <Suspense fallback={<BackdropSpinner />}>
        <StyledDownloadLink>
          <PDFDownloadLink document={document} fileName={`${fileName}.pdf`}>
            {({ loading }) =>
              loading ? <BackdropSpinner /> : `Download ${fileName} PDF`
            }
          </PDFDownloadLink>
        </StyledDownloadLink>
      </Suspense>
