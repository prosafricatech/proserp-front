...
function ShiftSummary({ paymentItems = [], shiftData = null }) {
  ...
  const calculateCashierMainLedgerAmount = (cashier) => {
    if (
      cashier.main_ledger_amount !== undefined &&
      cashier.main_ledger_amount !== null
    ) {
    ...
    selectedPumps.forEach((sel) => {
      ...
      if (reading) {
        const sold =
          sanitizedNumber(reading.closing) - sanitizedNumber(reading.opening) ||
          0;
    ...
    const filteredTransactionsSum = filteredCashTransactions.reduce(
      (sum, transaction) => sum + sanitizedNumber(transaction?.amount),
      0
    );
  ...
  const totalCollectedAmount = useMemo(() => {
    const totalPayments = paymentItems.reduce(
      (sum, item) => sum + sanitizedNumber(item?.amount),
      0
    );

    return (
      allCashiers.reduce(
        (sum, cashier) => sum + sanitizedNumber(cashier.collected_amount),
        0
      ) + totalPayments
    );
    ...
    allCashiers.forEach((cashier) => {
      ...
      if (mainLedger) {
        ...
        summary.push({
          ...
          ledgerName:
            ungroupedLedgerOptions.find((ledger) => ledger.id === mainLedger.id)
  ...
  const otherLedgersSummary = useMemo(() => {
    ...
    allCashiers.forEach((cashier) => {
      ...
      cashTransactions.forEach((transaction) => {
        const transactionLedgerId = transaction.ledger_id || transaction.id;
        const ledgerName = ungroupedLedgerOptions.find(
          (ledger) => ledger.id === transactionLedgerId
        )?.name;
    ...
    const totalOtherLedgerAmount = summary.reduce(
  ...
  const fuelVouchersAggregated = useMemo(() => {
    ...
    const totalVoucherAmount = summary.reduce(
      (sum, voucher) => sum + voucher.amount,
      0
    );
    const totalVoucherQuantity = summary.reduce(
      (sum, voucher) => sum + voucher.quantity,
      0
    );
  ...
  const totalExpectedCash = useMemo(() => {
    return allCashiers.reduce(
      (sum, cashier) => sum + cashierExpectedCash(cashier),
      0
    );
  ...
  const totalPumpReadings = useMemo(() => {
    ...
    allCashiers.forEach((cashier) => {
      ...
      selectedPumps.forEach((sel) => {
        ...
        if (reading) {
          const sold =
            sanitizedNumber(reading.closing) -
              sanitizedNumber(reading.opening) || 0;
  ...
  const combinedProductTotals = useMemo(() => {
    ...
    Object.keys(totalAdjustments.totals).forEach((productId) => {
      totals[productId] =
        (totals[productId] || 0) + totalAdjustments.totals[productId];
    });
  ...
  const financialSummary = useMemo(() => {
    ...
    let totalVouchersAmount =
      fuelVouchersAggregated.totalVoucherAmount ||
      totalFuelVouchers.totalAmount;
  ...
  const groupedAdjustments = useMemo(() => {
    ...
    const allAdjustments = shiftData.cashiers.flatMap(
      (c) => c.tank_adjustments
    );
    ...
    // 3. build final array
    const finalArray = Object.entries(grouped).map(([tankId, totalQty]) => {
      const tank = shiftData.shift_tanks.find((t) => t.id === Number(tankId));
      if (!tank) return null;
  ...
  }, [shiftData.cashiers, shiftData.shift_tanks, shiftData.fuel_prices]);
  ...
  const renderPaymentsSummary = () => {
    ...
    paymentItems.forEach((item) => {
      const debitLedger = ungroupedLedgerOptions.find(
        (l) => l.id === item.debit_ledger_id
      );
      const creditLedger = ungroupedLedgerOptions.find(
        (l) => l.id === item.credit_ledger_id
      );
      // Paid To
      if (debitLedger) {
        if (!paidToMap.has(debitLedger.id)) {
          paidToMap.set(debitLedger.id, {
            name: debitLedger.name,
            total: 0,
            payers: new Set(),
          });
        }
      ...
      <CardContent>
        ...
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 12 }}>
            <Paper
              elevation={0}
              sx={{ p: 2, bgcolor: 'info.50', borderRadius: 1, mt: 2 }}
            >
              <Typography
                variant='subtitle2'
                color='info.dark'
                ...
                <>
                  <TableContainer sx={{ maxHeight: 200 }}>
                    <Table size='small'>
                      ...
                      <TableBody>
                        {fuelVouchersAggregated.summary.map(
                          (voucher, index) => (
                            <TableRow key={index}>
                              <TableCell>
                                <Typography variant='body2'>
                    ...
                    <Grid container spacing={2}>
                      <Grid size={{ xs: 6 }}>
                        ...
                        >
                          ...
                          <Typography variant='body2' fontWeight='bold'>
                            {fuelVouchersAggregated.totalVoucherQuantity.toLocaleString()}{' '}
                            L
                          </Typography>
                        </Box>
                      </Grid>
          ...
          <Grid size={{ xs: 12 }}>
            ...
            >
              ...
              <Grid container spacing={2}>
                ...
                <Grid size={{ xs: 12, md: 4 }}>
                  <Box
                    sx={{ p: 1.5, bgcolor: 'secondary.50', borderRadius: 1 }}
                  >
                    <Typography
                      variant='caption'
                      color='textSecondary'
          ...
          <Table size='small'>
            ...
            <TableBody>
              ...
              >
                ...
                <TableCell align='right'>
                  {Object.values(totalPumpReadings)
                    .reduce((sum, qty) => sum + (qty || 0), 0)
                    .toLocaleString()}
                </TableCell>
                <TableCell align='right'>
                  <Typography>
                    {Object.values(totalAdjustments.totals)
                      .reduce((sum, qty) => sum + (qty || 0), 0)
                      .toLocaleString()}
                  </Typography>
                </TableCell>
                <TableCell align='right'>
                  {Object.values(combinedProductTotals)
                    .reduce((sum, qty) => sum + (qty || 0), 0)
                    .toLocaleString()}
                </TableCell>
        ...
        {Object.keys(totalAdjustments.totals).length > 0 && (
          <Box sx={{ mt: 2, p: 1, bgcolor: 'action.hover', borderRadius: 1 }}>
            <Typography variant='caption' display='block'>
              Adjustments: Gain (+)
              {totalAdjustments.adjustmentsByOperator['+'].toLocaleString()}L /
              Loss (-)
              {totalAdjustments.adjustmentsByOperator['-'].toLocaleString()}L
            </Typography>
          </Box>
  ...
  const renderAdjustmentsSummary = () => (
    <Card variant='outlined' sx={{ mb: 3 }}>
      <CardContent>
        <Typography
          variant='h6'
        ...
        <TableContainer>
          <Table size='small'>
            ...
            <TableBody>
              {shiftData?.cashiers?.length &&
                groupedAdjustments.map((adj, index) => {
                  totalAmt += adj.total_amount;
                  return (
                    <TableRow key={index}>
                      ...
                      <TableCell align='left'>
                        <Typography>{adj.product_name}</Typography>
                      </TableCell>
                      <TableCell align='right'>
                        <Typography>
              ...
              >
                ...
                <TableCell align='right'>
                  <Typography>
                    {Object.values(totalAdjustments.totals).reduce(
                      (sum, qty) => sum + (qty || 0),
                      0
                    ) > 0
                      ? '+' +
          ...
          <>
              ...
              <Table size='small'>
                ...
                <TableBody>
                  ...
                  >
                    ...
                    <TableCell align='right'>
                      {Object.values(totalFuelVouchers.totals)
                        .reduce((sum, qty) => sum + (qty || 0), 0)
                        .toLocaleString()}
                    </TableCell>
                    <TableCell align='right'>-</TableCell>
            ...
            <Box sx={{ mt: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Typography variant='caption' color='textSecondary'>
                Across {allCashiers.length} cashier
                {allCashiers.length !== 1 ? 's' : ''}
              </Typography>
            </Box>
          </>
        )}
  ...
  const totalPayments = useMemo(
    () => paymentItems.reduce((sum, item) => sum + sanitizedNumber(item?.amount), 0),
    [paymentItems]
  );
      ...
      <CardContent>
        ...
        <Grid container spacing={2}>
              ...
              >
                ...
                <TableContainer>
                  <Table size='small'>
                    ...
                    <TableBody>
                      {profitLossSummary.cashierResults.map(
                        (cashier, index) => (
                          <TableRow key={index}>
                            <TableCell>
                              <Typography fontWeight='medium'>
                            ...
                            <TableCell align='right'>
                              <Typography>
                                {formatMoney(cashier.expectedCash)}
                              </Typography>
                            </TableCell>
                            <TableCell align='right'>
                              <Typography color='info.main'>
                                {formatMoney(cashier.collectedAmount)}
                              </Typography>
                            </TableCell>
                            <TableCell align='right'>
                              <Typography
                              ...
                              >
                                {cashier.profitLoss === 0
                                  ...
                                  : cashier.profitLoss > 0
                                    ? '+'
                                    : '-'}
                                {formatMoney(Math.abs(cashier.profitLoss))}
                              </Typography>
                            </TableCell>
                            <TableCell align='center'>
                              <Chip
          ...
          >
            <Paper
              elevation={0}
              sx={{ p: 2, bgcolor: 'info.50', borderRadius: 1, mb: 2 }}
            >
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            ...
            >
              ...
              >
                <Typography>
                  Expected Cash{currencyCode ? ` (${currencyCode})` : ''}:
                </Typography>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography>
                  Actual Collected{currencyCode ? ` (${currencyCode})` : ''}:
                </Typography>
                <Typography
                  variant='h4'
                  color='info.main'
              ...
              >
                <Typography fontWeight='bold'>
                  Difference{currencyCode ? ` (${currencyCode})` : ''}:
                </Typography>
                <Typography
                  variant='h5'
                  fontWeight='bold'
                ...
                >
                  {profitLossSummary.netProfitLoss === 0
                    ...
                    : profitLossSummary.netProfitLoss > 0
                      ? '+'
                      : '-'}
                  {formatMoney(Math.abs(profitLossSummary.netProfitLoss))}
                </Typography>
              </Box>
            </Paper>
          </Grid>
  ...
  const renderShiftOverview = () => (
    <Card variant='outlined' sx={{ mb: 3 }}>
      <CardContent>
        ...
        {/* Balance Status */}
        <Box
          sx={{ mt: 3, p: 2, borderRadius: 1, bgcolor: 'background.default' }}
        >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Typography>
                {profitLossSummary.netProfitLoss === 0
                  ? 'Perfectly Balanced'
                  : profitLossSummary.netProfitLoss > 0
                    ? `Over: +${profitLossSummary.netProfitLoss.toLocaleString()}`
    ...
    <div>
      ...
      {allCashiers.length === 0 && (
        <Alert severity='info' sx={{ mt: 2 }}>
          No cashiers added yet. Please add cashiers in the "Cashiers Records"
          tab.
        </Alert>
      )}
    </div>
