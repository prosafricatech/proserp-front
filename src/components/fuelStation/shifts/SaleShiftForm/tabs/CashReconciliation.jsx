...
function CashReconciliation({
  ...
  const { setValue, errors, watch, trigger } = useFormContext();
  ...
  const collectionLedgerId = useWatch({
    name: `cashiers.${cashierIndex}.collection_ledger_id`,
  });
  ...
  const cashierLedgers = getCashierLedgers
    ? getCashierLedgers(cashierIndex)
    : [];
  const actualMainLedgerAmount =
    watch(`cashiers.${cashierIndex}.main_ledger_amount`) || 0;

  const [initialized, setInitialized] = useState(false);
  const [hasLoadedSavedTransactions, setHasLoadedSavedTransactions] =
    useState(false);
  ...
  useEffect(() => {
    if (
      cashierData?.other_transactions?.length > 0 &&
      !hasLoadedSavedTransactions &&
      !initialized
    ) {
      const otherTransactions = cashierData.other_transactions.filter(
        (transaction) => {
          const transactionLedgerId =
            transaction.ledger_id ||
            transaction.debit_ledger?.id ||
            transaction.id;
  ...
  useEffect(() => {
    if (!initialized) {
      const existingOtherTransactions =
        watch(`cashiers.${cashierIndex}.other_transactions`) || [];
      ...
      if (mainLedgerId) {
        ...
        if (
          ...
          setValue(
            `cashiers.${cashierIndex}.main_ledger`,
            {
              id: ledgerObj.id,
              name: ledgerObj.name,
        ...
        setValue(
          `cashiers.${cashierIndex}.main_ledger_id`,
          cashierMainLedger.id,
          {
            shouldValidate: true,
            shouldDirty: true,
          }
  ...
  const fuelVoucherTotals = useMemo(() => {
    ...
    localFuelVouchers.forEach((voucher) => {
      ...
      const price =
        productPrices.find((p) => p?.product_id === productId)?.price || 0;
  ...
  const productTotals = useMemo(() => {
    ...
    fuel_pumps?.forEach((pump) => {
      ...
      if (!productId) return;
      const reading = localPumpReadings.find(
        (r) => r?.fuel_pump_id === pump.id
      );
  ...
  const { grandFuelVoucherTotal, grandProductsTotal, cashRemaining } =
    useMemo(() => {
      const voucherTotal = Object.values(fuelVoucherTotals).reduce(
        (sum, v) => sum + (v || 0),
        0
      );

      const productsTotal =
        cashierPumpProducts?.reduce((sum, product) => {
  ...
  const totalProductQty = useMemo(() => {
    ...
    return totals;
  }, [localPumpReadings, fuel_pumps]);

  const adjustmentQty = useMemo(() => {
    const totals = {};

    localAdjustments.forEach((adj) => {
      const prodId = adj.product_id;
      if (!prodId) return;
  ...
  const filteredTransactionsSum = React.useMemo(() => {
    return (
      filteredCashTransactions.reduce(
        (sum, transaction) => sum + sanitizedNumber(transaction?.amount || 0),
        0
      ) || 0
    );
  ...
  useEffect(() => {
    if (
      mainLedgerId &&
      calculatedMainLedgerAmount !== null &&
      calculatedMainLedgerAmount !== undefined
    ) {
      setValue(
        `cashiers.${cashierIndex}.main_ledger_amount`,
        calculatedMainLedgerAmount,
        {
          shouldValidate: true,
          shouldDirty: true,
        }
  ...
  const isShort =
    (isZeroCollected && expectedCash < 0) || isZeroCollected || profitLoss < 0;
  const profitLossLabel = isShort ? 'Short' : 'Over';
  const profitLossIcon = isShort ? (
  ...
  const availableLedgers = useMemo(() => {
    return (cashierLedgers || []).filter(
      (ledger) => ledger.id !== mainLedgerId
    );
  ...
  const removeCashTransaction = React.useCallback(
    (idx) => {
      setValue(
        `cashiers.${cashierIndex}.other_transactions`,
  ...
  const updateCashTransaction = React.useCallback(
    (idx, field, value) => {
  ...
  const handleCollectedAmountChange = (value) => {
    ...
    if (/^0[0-9]+/.test(value)) {
      ...
      alert(
        'Collected Amount should not start with 0. Please enter the correct amount.'
      );
      return;
    }
    if (value === '' || value === null) {
    ...
    if (
      collectionLedgerId &&
      (sanitizedValue === 0 || sanitizedValue === '0')
    ) {
                ...
                <Table size='small'>
                  ...
                  <TableBody>
                    {cashierPumpProducts?.map((product) => {
                      const prodQty = totalProductQty[product.id] || 0;
                      const adjQty = adjustmentQty[product.id] || 0;
                      const qty = productTotals[product.id] || 0;
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, md: 4 }}>
                  <Autocomplete
                    size='small'
                    options={
                      cashierLedgers.filter((l) => l.ledger_group_id === 13) ||
                      []
                    }
                    ...
                    value={
                      mainLedgerId
                        ? cashierLedgers
                            .filter((l) => l.ledger_group_id === 13)
                            .find((l) => l.id === mainLedgerId) || null
                        : null
                    }
                    onChange={(_, newValue) => {
                      ...
                      if (newValue) {
                        ...
                        setValue(`cashiers.${cashierIndex}.main_ledger`, null, {
                          shouldValidate: true,
                          shouldDirty: true,
                        });
                      }
                    ...
                    renderInput={(params) => (
                      <TextField
                        ...
                        label='Main Ledger'
                        error={
                          !!errors?.cashiers?.[cashierIndex]?.main_ledger_id
                        }
                        helperText={
                          errors?.cashiers?.[cashierIndex]?.main_ledger_id
                            ?.message
                ...
                {filteredCashTransactions.map((transaction, idx) => {
                  ...
                  const ledgerObj = availableLedgers.find(
                    (l) => l.id === ledgerId
                  );
                    ...
                    <React.Fragment key={stableKey}>
                        ...
                        <Autocomplete
                          ...
                          renderInput={(params) => (
                            <TextField
                              ...
                              label='Other Ledger'
                              error={
                                !!errors?.cashiers?.[cashierIndex]
                                  ?.other_transactions?.[originalIdx]?.ledger_id
                              }
                              helperText={
                                errors?.cashiers?.[cashierIndex]
                                  ?.other_transactions?.[originalIdx]?.ledger_id
                      ...
                      <Grid size={{ xs: 12, md: 5 }}>
                        <TextField
                          ...
                          error={
                            !!errors?.cashiers?.[cashierIndex]
                              ?.other_transactions?.[originalIdx]?.narration
                          }
                          helperText={
                            errors?.cashiers?.[cashierIndex]
                              ?.other_transactions?.[originalIdx]?.narration
                      ...
                      <Grid size={{ xs: 11, md: 2 }}>
                        <TextField
                          ...
                          error={
                            !!errors?.cashiers?.[cashierIndex]
                              ?.other_transactions?.[originalIdx]?.amount
                          }
                          helperText={
                            errors?.cashiers?.[cashierIndex]
                              ?.other_transactions?.[originalIdx]?.amount
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <Grid container spacing={2}>
                  <Grid size={{ xs: 12, md: 6 }}>
                    <Autocomplete
                      options={(collection_ledgers || []).filter(
                        (l) => l.ledger_group_id === 13
                      )}
                      getOptionLabel={(opt) => opt?.name || ''}
                      size='small'
                      value={
                        (collection_ledgers || [])
                          .filter((l) => l.ledger_group_id === 13)
                          .find((l) => l.id === collectionLedgerId) || null
                      }
                      onChange={(_, newValue) => {
                        setValue(
                          `cashiers.${cashierIndex}.collection_ledger_id`,
                          newValue?.id ?? null,
                          { shouldValidate: true, shouldDirty: true }
                        );
                        trigger(
                      ...
                      renderInput={(params) => (
                        <TextField
                          ...
                          error={
                            !!errors?.cashiers?.[cashierIndex]
                              ?.collection_ledger_id || !!manualLedgerError
                          }
                          helperText={
                            errors?.cashiers?.[cashierIndex]
                              ?.collection_ledger_id?.message ||
                            manualLedgerError
                          }
                        />
                      )}
                  ...
                  <Grid size={{ xs: 12, md: 6 }}>
                    <TextField
                      ...
                      onChange={(e) =>
                        handleCollectedAmountChange(e.target.value)
                      }
                      error={
                        !!collectionLedgerId &&
                        (!!errors?.cashiers?.[cashierIndex]?.collected_amount ||
                          !!manualCollectedError)
                      }
                      helperText={
                        collectionLedgerId
                          ? errors?.cashiers?.[cashierIndex]?.collected_amount
                              ?.message || manualCollectedError
                          : ''
                      }
                      InputProps={{
                        inputComponent: CommaSeparatedField,
              ...
              <TableContainer>
                <Table size='small'>
                  <TableBody>
                    ...
                    {/* Over/Short Display */}
                    <TableRow
                      sx={{ bgcolor: isShort ? 'error.50' : 'success.50' }}
                    >
                      <TableCell sx={{ fontWeight: 'bold' }}>
                        <Box
                          sx={{
                        ...
                        >
                          {profitLossIcon}
                          <Typography color={profitLossColor}>
                            {profitLossLabel}
                          </Typography>
                        </Box>
                      </TableCell>
                      <TableCell
                        align='right'
                        sx={{ fontWeight: 'bold', color: profitLossColor }}
                      >
