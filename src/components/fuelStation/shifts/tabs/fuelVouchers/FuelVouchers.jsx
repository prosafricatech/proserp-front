...
function FuelVouchers({ index = -1, setShowForm = null, fuelVoucher, productPrices }) {
  ...
  const product = fuelVoucher && productOptions.find((p) => p.id === fuelVoucher?.product_id);
  const product_price = product && productPrices.find((price) => price?.product_id === product.id)?.price;
  ...
  const updateItems = async (item) => {
    ...
    if (index > -1) {
      ...
      await setFuelVouchers((prev) => [...prev, item]);
    }
    reset();
    setIsAdding(false);
        ...
        <Grid size={{ xs: 12, md: 4, lg: !watch('stakeholder_id') ? 4 : 3 }}>
          <Div sx={{ mt: 1 }}>
            <ProductSelect
              ...
              defaultValue={fuelVoucher && productOptions.find((p) => p.id === fuelVoucher.product_id)}
