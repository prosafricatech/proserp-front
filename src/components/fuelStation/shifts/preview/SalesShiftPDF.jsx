...
function SalesShiftPDF({ includeFuelVouchers, shiftData, stationName, organization, cashiers, fuel_pumps, tanks, productOptions }) {
  ...
  const calculateCashierTotals = (cashier) => {
    ...
    const totalProductsAmount = cashier.pump_readings?.reduce((total, pump) => {
      const productPrice = shiftData.fuel_prices.find(fp => fp.product_id === pump.product_id)?.price || 0;
    ...
    const adjustmentsAmount = cashier.tank_adjustments?.reduce((total, adj) => {
      const productPrice = shiftData.fuel_prices.find(fp => fp.product_id === adj.product_id)?.price || 0;
      if (adj.operator === '-') {
    ...
    const totalFuelVouchersAmount = cashier.fuel_vouchers?.reduce((total, fv) => {
      const productPrice = shiftData.fuel_prices.find(fp => fp.product_id === fv.product_id)?.price || 0;
    ...
    const cashRemaining = (totalProductsAmount + adjustmentsAmount) - totalFuelVouchersAmount;
  ...
  const overallTotals = shiftData.cashiers?.reduce((acc, cashier) => {
    const cashierTotals = calculateCashierTotals(cashier);
    return {
      totalProductsAmount: acc.totalProductsAmount + cashierTotals.totalProductsAmount,
      adjustmentsAmount: acc.adjustmentsAmount + cashierTotals.adjustmentsAmount,
      totalFuelVouchersAmount: acc.totalFuelVouchersAmount + cashierTotals.totalFuelVouchersAmount,
      cashTransactionsTotal: acc.cashTransactionsTotal + cashierTotals.cashTransactionsTotal,
  ...
  const mergeCashierPumpReadings = (pumpReadings) => {
    const merged = pumpReadings.reduce((acc, pump) => {
      if (!acc[pump.product_id]) {
        ...
        acc[pump.product_id].quantity += (pump.closing || 0) - (pump.opening || 0);
    ...
    >
      <Page size="A4" style={pdfStyles.page}>
        ...
        <View style={{ ...pdfStyles.tableRow, marginBottom: 12 }}>
          <View style={{ flex: 1, padding: 2 }}>
            ...
            <Text style={{ ...pdfStyles.midInfo }}>
              {cashiers?.find(c => c.id === shiftData.sales_outlet_shift_id)?.name || 'N/A'}
            </Text>
          </View>
          <View style={{ flex: 1, padding: 2 }}>
        ...
        {shiftData.cashiers?.map((cashier, cashierIndex) => {
          const cashierTotals = calculateCashierTotals(cashier);
          const mergedReadings = mergeCashierPumpReadings(cashier.pump_readings || []);

          return (
            <View key={cashier.id} style={{ marginBottom: 20, pageBreakInside: 'avoid' }}>
              {/* Cashier Header */}
              <View style={{ marginBottom: 8, padding: 8, backgroundColor: mainColor, borderRadius: 4 }}>
                <Text style={{ fontSize: 14, color: contrastText, fontWeight: 'bold', textAlign: 'center' }}>
              ...
              <View style={{ marginBottom: 12 }}>
                <View style={pdfStyles.table}>
                  ...
                  <View style={pdfStyles.tableRow}>
                    <Text style={{ ...pdfStyles.tableCell, flex: 2 }}>Fuel Vouchers Total</Text>
                    <Text style={{ ...pdfStyles.tableCell, flex: 1, textAlign: 'right' }}>
                  ...
                  <View style={pdfStyles.tableRow}>
                    ...
                    <Text style={{ ...pdfStyles.tableCell, flex: 1, textAlign: 'right', fontWeight: 'bold', color: cashierTotals.cashRemaining < 0 ? '#FF0000' : '#000000' }}>
              ...
              {(cashier.main_ledger || cashier.cash_transactions?.length > 0) && (
                <View style={{ marginBottom: 12 }}>
                  ...
                  <View style={pdfStyles.table}>
                    ...
                    {cashier.main_ledger && (
                      <View style={pdfStyles.tableRow}>
                        <Text style={{ ...pdfStyles.tableCell, backgroundColor: '#FFFFFF', flex: 3 }}>
                          {cashier.main_ledger.name || `Ledger ${cashier.main_ledger.id}`}
                        </Text>
                        <Text style={{ ...pdfStyles.tableCell, backgroundColor: '#FFFFFF', flex: 1.5, textAlign: 'right' }}>
                    ...
                    {cashier.cash_transactions?.map((transaction, index) => {
                      ...
                      return (
                        <View key={index} style={pdfStyles.tableRow}>
                          <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? lightColor : '#FFFFFF', flex: 3 }}>
                            {ledger.name}
                          </Text>
                          <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? lightColor : '#FFFFFF', flex: 1.5, textAlign: 'right' }}>
                    ...
                    {/* Total */}
                    <View style={pdfStyles.tableRow}>
                      <Text style={{ ...pdfStyles.tableHeader, ...pdfStyles.tableCell, backgroundColor: mainColor, color: contrastText, flex: 3, fontWeight: 'bold' }}>
                        Total Distributed
                      </Text>
                ...
                <View style={{ marginBottom: 12 }}>
                  ...
                  <View style={pdfStyles.table}>
                    <View style={pdfStyles.tableRow}>
                      ...
                      <Text style={{ ...pdfStyles.tableHeader, backgroundColor: mainColor, color: contrastText, flex: 2 }}>Client</Text>
                    ...
                    {cashier.fuel_vouchers.map((fv, index) => {
                      const product = productOptions?.find(p => p.id === fv.product_id);
                      const price = shiftData.fuel_prices.find(p => p.product_id === fv.product_id)?.price || 0;
                      ...
                      return (
                        <View key={index} style={pdfStyles.tableRow}>
                          ...
                          <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 1, textAlign: 'right' }}>
                            {fv.quantity.toLocaleString('en-US', { minimumFractionDigits: 3, maximumFractionDigits: 3 })}
                          </Text>
                          <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 1, textAlign: 'right' }}>
                ...
                <View style={{ marginBottom: 12 }}>
                  ...
                  <View style={pdfStyles.table}>
                    ...
                    {cashier.tank_adjustments.map((adj, index) => {
                      const product = productOptions?.find(p => p.id === adj.product_id);
                      ...
                      return (
                        <View key={index} style={pdfStyles.tableRow}>
                          ...
                          <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 1, textAlign: 'right' }}>
                            {adj.quantity.toLocaleString('en-US', { minimumFractionDigits: 3, maximumFractionDigits: 3 })}
                          </Text>
                        </View>
                      );
                    })}
          ...
          <View style={{ marginTop: 20, pageBreakInside: 'avoid' }}>
            ...
            <View style={pdfStyles.table}>
              ...
              <View style={pdfStyles.tableRow}>
                ...
                <Text style={{ ...pdfStyles.tableCell, flex: 1, textAlign: 'right' }}>
                  {overallTotals.netSales.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </Text>
              </View>
              <View style={pdfStyles.tableRow}>
                <Text style={{ ...pdfStyles.tableCell, flex: 2 }}>Total Fuel Vouchers</Text>
                <Text style={{ ...pdfStyles.tableCell, flex: 1, textAlign: 'right' }}>
                  {overallTotals.totalFuelVouchersAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </Text>
              </View>
              <View style={pdfStyles.tableRow}>
                <Text style={{ ...pdfStyles.tableCell, flex: 2, fontWeight: 'bold' }}>Total Cash Remaining</Text>
                <Text style={{ ...pdfStyles.tableCell, flex: 1, textAlign: 'right', fontWeight: 'bold', color: overallTotals.cashRemaining < 0 ? '#FF0000' : '#000000' }}>
                  {overallTotals.cashRemaining.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </Text>
              </View>
            </View>
          </View>
        ...
        {/* ================= DIPPING SECTION ================= */}
        {(shiftData.opening_dipping?.readings?.length > 0 || shiftData.closing_dipping?.readings?.length > 0) && (
          <View style={{ marginTop: 20, pageBreakInside: 'avoid' }}>
            <View style={{ marginBottom: 8, padding: 8, backgroundColor: mainColor, borderRadius: 4 }}>
              <Text style={{ fontSize: 14, color: contrastText, fontWeight: 'bold', textAlign: 'center' }}>
                Dipping Records
              ...
              <View style={{ marginBottom: 12 }}>
                ...
                <View style={pdfStyles.table}>
                  <View style={pdfStyles.tableRow}>
                    ...
                    <Text style={{ ...pdfStyles.tableHeader, backgroundColor: mainColor, color: contrastText, flex: 1 }}>Deviation</Text>
                  </View>
                  {shiftData.opening_dipping.readings.map((od, index) => {
                    const product = productOptions?.find(p => p.id === od.product_id);
                    return (
                      <View key={index} style={pdfStyles.tableRow}>
                        <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 1.5 }}>
                        ...
                        <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 1, textAlign: 'right' }}>
                          {(od.deviation || 0).toLocaleString('en-US', { minimumFractionDigits: 3 })}
                        </Text>
                      </View>
                    );
                  })}
              ...
              <View style={{ marginBottom: 12 }}>
                ...
                <View style={pdfStyles.table}>
                  ...
                  {shiftData.closing_dipping.readings.map((cd, index) => {
                    const product = productOptions?.find(p => p.id === cd.product_id);
                    return (
                      <View key={index} style={pdfStyles.tableRow}>
                        <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 1.5 }}>
                        ...
                        <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 1, textAlign: 'right' }}>
                          {(cd.deviation || 0).toLocaleString('en-US', { minimumFractionDigits: 3 })}
                        </Text>
                      </View>
                    );
                  })}
