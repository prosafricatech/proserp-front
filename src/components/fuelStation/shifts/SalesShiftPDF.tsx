...
import { SalesShiftData } from './SalesShiftTypes';
import { Organization } from '@/types/auth-types';
import { Product, PumpReading } from '../dippings/DippingsTypes';

interface SalesShiftPDFProps {
  includeFuelVouchers: boolean;
  shiftData: SalesShiftData;
  organization: Organization;
  shift_teams: Array<{ id: number; name: string }>;
  fuel_pumps: Array<{ id: number; name: string }>;
  tanks: Array<{ id: number; name: string }>;
  productOptions: Product[];
}

const SalesShiftPDF: React.FC<SalesShiftPDFProps> = ({
  includeFuelVouchers,
  shiftData,
  organization,
  shift_teams,
  ...
  const totalFuelVouchersAmount = shiftData.fuel_vouchers.reduce((total, fuelVoucher) => {
    ...
    const mergePumpReadings = (
    pumpReadings: PumpReading[]
    ): Array<PumpReading & { quantity: number }> => {
    const merged = pumpReadings.reduce((acc, pump) => {
        ...
        return acc;
    }, {} as Record<number, PumpReading & { quantity: number }>);
  ...
  return (
    ...
    >
        <Page size="A4" style={pdfStyles.page}>
                    ...
                    <View style={{ ...pdfStyles.table, padding: 3, flex: 1, marginTop: 5 }}>
                        ...
                        {shiftData?.adjustments?.map((adjustment, index) => (
                            <View key={index} style={pdfStyles.tableRow}>
                                ...
                                <Text style={{ ...pdfStyles.tableCell, flex: 1 }}> {tanks.find(t => t.id === adjustment.tank_id)?.name || '—'}</Text>
            ...
            <View style={{ ...pdfStyles.tableRow }}>
                {shiftData.opening_dipping?.readings && shiftData.opening_dipping.readings.length >> 0 &&
                ...
                {shiftData.closing_dipping?.readings && shiftData.closing_dipping.readings.length >0 &&
                    <View style={{ ...pdfStyles.table, padding: 3, flex: 1,marginTop:15 }}>
                        <View style={{ ...pdfStyles.tableRow,marginBottom: 1 }}>
                ...
                <View style={{ ...pdfStyles.table, minHeight: 230, marginTop: 20 }}>
                    ...
                    {shiftData.fuel_vouchers.map((fuelVoucher, index) => (
                        <View key={index} style={pdfStyles.tableRow}>
                            ...
                            <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 2.5 }}>{fuelVoucher.clientName}</Text>
                            ...
                            <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 1.5, textAlign: 'right' }}>{shiftData.fuel_prices?.find(product => product.product_id === fuelVoucher.product_id)?.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) ?? '0.00'}</Text>
                            <Text style={{ ...pdfStyles.tableCell, backgroundColor: index % 2 === 0 ? '#FFFFFF' : lightColor, flex: 1.5, textAlign: 'right' }}>{(fuelVoucher.quantity * (shiftData.fuel_prices?.find(product => product.product_id === fuelVoucher.product_id)?.price ?? 0)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</Text>
                        </View>
                    ))}
                    <View style={{ ...pdfStyles.tableRow}}>
