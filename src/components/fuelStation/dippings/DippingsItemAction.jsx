...
const EditDipping = ({dipping, setOpenEditDialog}) => {
  const {data:dippingData,isFetching} = useQuery(['showDippingDetails',{id:dipping.id}], () => fuelStationServices.showDippingDetails(dipping.id));
...
const DocumentDialog = ({openDocumentDialog, setOpenDocumentDialog, organization, dipping}) => {
  ...
  const { shift_teams, fuel_pumps } = activeStation;
...
const DippingsItemAction = ({ dipping}) => {
  ...
  const { mutate: deleteDipping } = useMutation(fuelStationServices.deleteDipping, {
    onSuccess: (data) => {
      queryClient.invalidateQueries(['stationDippings']);
