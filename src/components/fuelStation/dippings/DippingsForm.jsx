...
const DippingsForm = ({ setOpenDialog, dippingData }) => {
  ...
  const addDipping = useMutation(fuelStationServices.addDipping, {
    onSuccess: (data) => {
        setOpenDialog(false);
  ...
  const editDipping = useMutation(fuelStationServices.EditDipping, {
    onSuccess: (data) => {
        setOpenDialog(false);
        enqueueSnackbar(data.message, { variant: 'success' });
        queryClient.invalidateQueries(['stationDippings']);
                    ...
                    <Grid item xs={12} marginTop={2}>
                        {fields.map((field, index) => (
                            <Grid key={field.id} container columnSpacing={1}>
                                <Grid item xs={fields.length > 1 ? 11 : 12} marginBottom={0.5}>
                                    <Divider />
                                    <Grid container columnSpacing={1}>
